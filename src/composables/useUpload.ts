...
const useUpload = () => {
  ...
  const rightIsSet = (_right: Rights) => {
    console.log(
      `_right`,
      _right,
      uploadState.liscense,
      License[_right] === uploadState.liscense
    );
  ...
  const setBase64Image = (_base64String: string | null) => {
    _base64String === null
      ? (uploadState.base64Image = NO_IMAGE_PATH)
      : (uploadState.base64Image = _base64String);
  };
  ...
  const getMediafiles = async (_entity: Entity) => {
    const mediafiles = [];
    const status = getMetadataOfTypeFromEntity(
      _entity,
    ...
    if (status !== undefined) {
      ...
      switch (activeKey) {
        ...
        default:
          _entity.mediafiles ? mediafiles.push(..._entity.mediafiles) : null;
          _entity.nonPublicMediafiles
            ? mediafiles.push(..._entity.nonPublicMediafiles)
            : null;
          break;
      }
    }
  ...
  const upload = async (_client: ApolloClient<NormalizedCacheObject>) => {
    ...
    const { mutate } = provideApolloClient(_client)(() =>
      useMutation(UploadMediafileDocument)
    );
    ...
    const res = await mutate({
      ...
      relations: getRelationsForUpload(
        uploadState.relations
      ) as Array<RelationInput>,
      metadata: getMetadataForUpload(
        uploadState.metadata,
        Publication.Validate,
        UserAction.Created
      ) as Array<MetadataInput>,
    });
  ...
  const getAssetStateObject = (
    _state: AssetState
  ): Promise<AssetStateObject> => {
  ...
  const updateAsset = async (
    ...
    const { mutate } = provideApolloClient(_client)(() =>
      useMutation(UpdateEntityDocument)
    );
    const updated = await mutate({
      id: _id,
      metadata: getMetadataForUpload(
        uploadState.metadata,
        assetState.publication,
        assetState.action
      ) as Array<MetadataInput>,
      relations: getRelationsForUpload(
        uploadState.relations
