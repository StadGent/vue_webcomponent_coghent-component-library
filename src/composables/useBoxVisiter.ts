import { AddAssetToBoxVisiterDocument, AddFrameToStoryBoxVisiterDocument, AddStoryToBoxVisiterDocument, BoxVisiter, CreateBoxVisiterDocument, FrameInput, GetBoxVisiterByCodeDocument, Relation, RelationType, StoryInput } from '@/queries';
...
const useBoxVisiter = (_client: ApolloClient<NormalizedCacheObject>): UseBoxVisiter => {
  ...
  const create = async (_storyId: string) => {
    const { fetchMore } = apolloProvider(() => useQuery(CreateBoxVisiterDocument, { storyId: _storyId }))
  ...
  const getByCode = async (_code: string) => {
    const { fetchMore } = apolloProvider(() => useQuery(GetBoxVisiterByCodeDocument, { code: _code }))
  ...
  const addStoryToVisiter = async (_code: string, _storyInput: StoryInput) => {
    const { mutate } = apolloProvider(() => useMutation(AddStoryToBoxVisiterDocument))
  ...
  const addFrameToStory = async (_code: string, _frameInput: FrameInput) => {
    const { mutate } = apolloProvider(() => useMutation(AddFrameToStoryBoxVisiterDocument))
  ...
  const addAssetToBoxVisiter = async (_code: string, _assetId: string, _type: RelationType.Visited | RelationType.InBasket) => {
    ...
    const { mutate } = apolloProvider(() => useMutation(AddAssetToBoxVisiterDocument))
    const updated = await mutate({ code: _code, assetId: _assetId, type: _type })
