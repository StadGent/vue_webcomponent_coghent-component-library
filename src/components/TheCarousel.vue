<template>
  <!--Fullscreen modal-->
  <base-modal :modalState="openIIIFModal" :large="true" class="z-50" @hide-modal="closeFullscreenModal">
    <section class="h-large flex relative w-full">
      ...
      <AudioPlayer
        v-if="mediafiles[selectedIndex].mediatype.audio ? mediafiles[selectedIndex].mediatype.audio : mediafiles[selectedIndex].filename.includes('.wav' || '.mp3')"
  ...
  <div class="flex items-center">
    <div class="z-0 mt-24 w-1/4 transform -translate-x-1/4">
      <lazy-load-image extraClass="opacity-70" :url="source[getPrevImage()].imageUrl" :fallBackUrl="source[getPrevImage()].fallBackUrl" v-if="source.length > 2" />
    </div>
    <div class="flex flex-col items-center w-3/4 relative">
      ...
      <copyright-tab class="absolute top-0 right-0 w-full h-full" :infotext="infotext" :mediafiles="mediafiles" :selectedIndex="selectedIndex" @openingCcmodal="openCCModal" :showMoreInfo="!isTouch" />
      <lazy-load-image extraClass="z-10" :url="source[selectedIndex].imageUrl" :fallBackUrl="source[selectedIndex].fallBackUrl" />
    ...
    <div class="z-0 mt-24 w-1/4 transform translate-x-1/4">
      <lazy-load-image extraClass="opacity-70" :url="source[getNextImage()].imageUrl" :fallBackUrl="source[getNextImage()].fallBackUrl" v-if="source.length > 1" />
    </div>
  </div>
</template>
<script lang="ts">
