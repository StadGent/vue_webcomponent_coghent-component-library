<template>
  <label class="flex block my-2 text-center text-text-dark py-2 shadow-lg pl-2 pr-8 w-full min-w-48 text-neutral-700 text-xl focus:outline-none rounded-full bg-text-white">
    <span v-if="label" class="ml-1 px-4 items-center font-bold text-sm rounded-full bg-accent-purple text-text-white hidden sm:flex">{{ label }}</span>
    ...
    <a @click="onClick" class="flex items-center pt-2 pb-2 cursor-pointer hover:text-accent-purple">
      <svg class="mr-2 stroke-current float-right sm:float-none" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
     ...
     <p class="hidden sm:flex"> {{searchLabel}}</p>
    </a>
  </label>
</template>
